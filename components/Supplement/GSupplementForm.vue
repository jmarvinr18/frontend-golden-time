<template>
  ...
  <div class="is-desktop g-auth-signup-form mt-2">
    <div class="g-form-group pb-5">
      <div class="mb-4 g-form-input">
        <label for="g-auth-form-1" class="form-label bg-white ms-2 px-2"
          >{{ $t(`BrandLabel`) }}*</label
        >
        <input
          type="text"
      ...
      <div class="mb-4 g-form-input">
        <label for="g-auth-form-2" class="form-label bg-white ms-2 px-2"
          >{{ $t(`ProductNameLabel`) }}*</label
        >
        <input
          type="text"
          class="form-control form-control-lg"
      ...
      <div class="mb-4 g-form-input">
        <div class="mb-4 g-form-input">
          ...
          >
            <option v-for="(opt, key) in typeOpt" :value="opt.value">
              {{ opt.label }}
            </option>
          </select>
        </div>
      ...
      <div class="mb-4 g-form-input">
        ...
        <input
          ...
          class="form-control form-control-lg"
          id="g-auth-form-1"
          :placeholder="$t('FlavourPlaceholder')"
          v-model="supplementForm.flavor"
          maxlength="25"
        />
      </div>
      <div class="mb-4 g-form-input">
        ...
        <input
          ...
          v-model="supplementForm.price"
        />
      </div>
      <div class="mb-4 g-form-input">
        <label for="g-auth-form-1" class="form-label bg-white ms-2 px-2"
          >{{ $t(`ComponentLabel`) }}*</label
        >
        <input
          type="text"
          class="form-control form-control-lg"
          ...
          maxlength="25"
          v-model="supplementForm.ingredients"
        />
      </div>
      <div class="mb-4 g-form-input">
        <div class="mb-4 g-form-input">
          ...
          <select
            class="form-select form-select-lg"
            aria-label="Default select example"
            v-model="supplementForm.serving_type"
          >
            <option v-for="(opt, key) in shapeOpt" :value="opt.value">
              {{ opt.label }}
            </option>
          </select>
        </div>
    ...
    <div class="w-100 py-3 d-flex justify-content-center text-center gap-3">
      <button
        class="btn btn-outline-secondary g-shadow rounded-pill px-4"
        @click="router.back()"
      >
        {{ $t("CancelButton") }}
      </button>
      <button
        class="btn btn-primary g-shadow rounded-pill px-4"
        @click="submitNow"
      >
        {{ $t("RegisterButton") }}
      </button>
  ...
  <div class="is-mobile g-auth-signup-form mt-2">
    ...
    <div class="w-100 py-3 d-flex justify-content-center text-center gap-2">
      <button
        class="w-50 btn btn-outline-secondary py-2 rounded-pill px-4"
        @click="router.back()"
      >
        {{ $t("CancelButton") }}
      </button>
      <button
        class="w-50 btn btn-primary rounded-pill py-2 px-4"
        @click="submitNow"
      >
        {{ $t("RegisterButton") }}
      </button>
...
export default defineComponent({
  setup(props) {
    ...
    const submitNow = () => {
      supplementForm.value.price = supplementForm.value.price;
      if (
        supplementForm.value.name &&
        supplementForm.value.supplement_type &&
        supplementForm.value.serving_type &&
        supplementForm.value.price &&
        supplementForm.value.ingredients &&
        supplementForm.value.url &&
        ...
        generalStore.setError(
          true,
          "Please make sure all required fields has been filled."
        );
      }
    };
    ...
    const submitSupplement = async () => {
      if (route.params.id) {
        await supplementStore
          .updateSupplement(supplementForm.value)
          .then((res) => {
            location.href = `/supplement/review/${route.params.id}`;
          });
      } else {
        await supplementStore
          .createSupplement(supplementForm.value)
          .then((res) => {
